...
def review_bundle(client, context):
    ...
    section_instruction = (
        ...
        +SECTION_COVERAGE_PROMPTS.get(section_call_id, '')
    ...
    prompt = (
        ...
        '판단에 중요한 불확실성은 summary에도 자연스럽게 드러낸다. 목차별 작성기준을 우선하며 기본적으로 요인당 3~5문장을 사용하되, '
        '소제목 책임 배분에서 별도 종합 문단을 지정한 경우에는 그 문단 길이 지시를 추가로 적용한다. '
