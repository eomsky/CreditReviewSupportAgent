...
이 단계에서 실제 심사 판단을 완성한다. 자료의 수치나 사건을 나열하는 데 그치지 않고 원인→현금흐름·재무부담→상환능력의 연결을 해당 근거가 허용하는 범위에서 설명한다. 반대 근거와 대안적 해석도 대조하고 조건부 판단을 명시한다. 내부 사고 전문은 출력하지 않는다.
reason은 한 문장, inquiry의 각 배열은 핵심 1~2개로 작성한다. summary는 판단에 필요한 길이로 작성하되 같은 사실을 반복하지 않는다. 복잡한 쟁점을 처리하지 않은 채 다른 항목을 참조하라고만 쓰거나 '제한적/안정적'이라고 근거 없이 결론내리지 않는다. 단일 비율의 임의 기준치만으로 위험등급을 단정하지 않는다. 충분한 원문이 있으면 형식적인 plan/search를 반복하지 않는다.
