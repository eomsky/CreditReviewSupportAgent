...
- 관련 요인을 묶어 호출하고 교차 검토 후 F30 종합의견을 작성한다. 출처 목록 길이 제한 등으로 출처 ID 무한 반복을 줄였다.
...
사용자 품질 기준: 업종과 거래에 맞게 분석하되, **근거 → 계산/비교 → 위험과 완화 → 상환능력 → 거래 조건/종합 판단**을 연결한다. 30개 요인은 현재 구현의 공통 축이며 모든 업종에 고정 목차를 기계적으로 강제하지 않는다. 제조업과 골프장 예시 모두가 기준이고 한 예시로 다른 예시를 대체하지 않는다.
...
> docs/HANDOVER_NEW_PC_2026-09-08.md와 사용자가 제공한 비공개 기준 문서를 읽고 여신 심사지원 에이전트 작업을 이어가줘. 먼저 개발 브랜치와 현재 Codespaces/Colab 연결 및 Drive 복구 자료를 확인해줘. 기본 모델을 유지하고 기존 프리징 운영 자산과 v1 태그는 보존해줘. 최종 목표는 예시 수준의 심사 품질과 신규 PDF 전체 보고서 120초 이내이며 아직 미달성이다. 우선 주요 주장 의미 검증과 prepared 요인별 추가 계산 경로의 현재 구현을 점검하고, 남은 오류를 재현할 최소 사례와 수정 범위를 정리해줘. 과거 5시간 학습 작업을 자동 재개하지 마.
