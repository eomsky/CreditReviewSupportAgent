...
response=json.loads((folder/'normalization-trial/attempt-001/response.json').read_text())
...
for f in facts:
    ...
    for field,column in [('period','period_cell'),('unit','unit_cell')]:
        ...
        raw=c.execute('SELECT raw_value FROM cells WHERE source_id=? AND column_name=? AND row_number=?',(cell['source_id'],match[1],int(match[2]))).fetchall()
        if len(raw)!=1:raise ValueError('Ambiguous metadata location')
        value=raw[0][0]
        if field=='unit':value=re.search(r'단위\s*:\s*([^)]*)',value)[1].strip()
        if f[field]!=value:raise ValueError('Metadata mismatch')
request_hash=hashlib.sha256((folder/'normalization.request.json').read_bytes()).hexdigest()
for f in facts:
...
(folder/'sql-query-example.json').write_text(json.dumps({'sql':query,'params':params,'rows':rows,'elapsed_seconds':elapsed,'normalization_rows_checked':len(facts),'scope':'fixed financial accounts table only','full_step1_completed':False},ensure_ascii=False,indent=2),encoding='utf-8')
